...
export default function SpeakingPracticePage() {
  ...
  const generateTopicSuggestions = (language) => {
    ...
    if (language === 'ja') {
      ...
      topics.push('Hispanic culture', 'Latin American food', 'Travel in Spanish-speaking countries');
    } else if (language === 'fr') {
  ...
  const startSpeakingPractice = async () => {
    ...
    try {
      ...
      const systemPrompt = createSystemPrompt(languageCode, proficiencyLevel, selectedTopic, nativeLanguage);
  ...
  const createSystemPrompt = (languageCode, proficiencyLevel, topic, nativeLanguage) => {
...
First, greet the student in ${languageName} and introduce yourself as their language tutor. Then ask a simple question about the topic to get the conversation started.`;
  ...
  const endSpeakingPractice = async () => {
    try {
      ...
      setError('Failed to save practice results, but your practice session was completed.');
    }
  };
  ...
  }, [callId, userChallengeId, callDuration, selectedTopic, languageCode, proficiencyLevel]);
      ...
      <div className="space-y-6">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Choose a topic to start practicing your {getLanguageName(languageCode)} speaking skills
          </p>
        </div>
        
        <div className="bg-white shadow-sm rounded-lg p-6 border border-gray-200">
          <h2 className="text-lg font-medium text-gray-900 mb-4">Select a Conversation Topic</h2>
          ...
          <div className="mt-6">
            <h3 className="text-md font-medium text-gray-700 mb-2">Or suggest your own topic:</h3>
            <div className="flex space-x-2">
              <input
                type="text"
                value={selectedTopic === availableTopics.includes(selectedTopic) ? '' : selectedTopic}
          ...
          <div className="mt-6 flex justify-end">
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-lg disabled:opacity-50"
            >
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-amber-800">
          ...
          <ul className="list-disc list-inside space-y-1 text-sm">
            ...
            <li>Speak naturally and try to express your thoughts in {getLanguageName(languageCode)}</li>
            <li>The tutor will adjust to your proficiency level and help you improve</li>
            <li>You'll receive feedback on your pronunciation, fluency, and grammar</li>
  ...
  const renderWebRTCCall = () => {
    return (
      <div className="space-y-6">
          ...
          <div className="aspect-w-16 aspect-h-9 relative">
            ...
            {!iframeLoaded && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-100">
                ...
                <p className="mt-4 text-gray-600">Connecting to your speaking practice session...</p>
              </div>
            )}
          </div>
            ...
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm text-gray-600">
                  Practicing: <span className="font-medium text-gray-900">{getLanguageName(languageCode)}</span>
                </p>
                <p className="text-sm text-gray-600">
                  Topic: <span className="font-medium text-gray-900">{selectedTopic}</span>
                </p>
              </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-amber-800">
          <h3 className="font-medium mb-2">Tips for a Great Practice Session</h3>
          <ul className="list-disc list-inside space-y-1 text-sm">
            ...
            <li>If you don't understand something, ask: "Could you repeat that?" or "Could you speak more slowly?"</li>
            <li>Take your time to formulate responses - pauses are natural in conversation</li>
          </ul>
        </div>
      </div>
    );
  ...
  const renderResults = () => {
    ...
    const feedbackStats = userFeedback ? [
      { name: "Pronunciation", value: userFeedback.pronunciation, color: "bg-blue-500" },
      { name: "Fluency", value: userFeedback.fluency, color: "bg-green-500" },
      { name: "Accuracy", value: userFeedback.accuracy, color: "bg-purple-500" },
      { name: "Overall", value: userFeedback.overall, color: "bg-amber-500" }
    ] : [];
    ...
    return (
      <div className="text-center">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            You've completed your {getLanguageName(languageCode)} speaking practice.
          </p>
        </div>
        
        <div className="bg-white shadow-sm rounded-lg p-8 border border-gray-200 mb-6">
          <div className="mb-6">
            <h2 className="text-lg font-bold text-gray-900 mb-4">Your Practice Summary</h2>
            
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-amber-50 rounded-lg p-4 border border-amber-100">
                <p className="text-sm text-amber-800">Practice Duration</p>
                <p className="text-xl font-bold text-amber-900">{formatTime(callDuration)}</p>
              </div>
              
              <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                <p className="text-sm text-blue-800">Language</p>
                <p className="text-xl font-bold text-blue-900">{getLanguageName(languageCode)}</p>
              </div>
              
              <div className="bg-green-50 rounded-lg p-4 border border-green-100">
                <p className="text-sm text-green-800">Topic</p>
                <p className="text-xl font-bold text-green-900 truncate">{selectedTopic}</p>
              </div>
            ...
            {userFeedback && (
              <div className="mt-8">
                <h3 className="text-md font-bold text-gray-800 mb-4">Performance Scores</h3>
                  ...
                  {feedbackStats.map((stat, index) => (
                    <div key={index}>
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="text-sm font-medium text-gray-700">{stat.value}/100</span>
                      </div>
                ...
                {userFeedback.suggestions && userFeedback.suggestions.length > 0 && (
                  <div className="mt-6 text-left">
                    <h3 className="text-md font-bold text-gray-800 mb-2">Improvement Suggestions</h3>
